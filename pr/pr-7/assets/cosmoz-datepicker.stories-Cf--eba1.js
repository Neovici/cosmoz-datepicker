...
`),Ot=(t,e,n)=>Ut(y`<cosmoz-tooltip-content>
		...
		</cosmoz-tooltip-content>`,t),Fe=(t,e)=>{const{for:n,heading:r,description:o,placement:a="top",delay:s=300,disabled:i=!1}=e,u=U(),p=!!(r||o)&&!i;k(()=>{if(!n||!p)return;const c=t.getRootNode(),d=c.adoptedStyleSheets??[];d.includes(pt)||(c.adoptedStyleSheets=[...d,pt]);const l=document.createElement("div");l.setAttribute("popover","manual"),l.setAttribute("role","tooltip"),l.classList.add("cosmoz-tooltip-popover"),t.after(l),u.current=l,Ot(l,r,o);const f=`[name="${n}"]`,h=`--tooltip-anchor-${n}`;let m;const b=$=>{i||(clearTimeout(m),$.style.anchorName=h,l.style.positionAnchor=h,l.style.positionArea=a,m=window.setTimeout(()=>l.showPopover(),s))},w=()=>{clearTimeout(m),l.hidePopover()},x=$=>{const M=$.target.closest?.(f);M&&b(M)},St=$=>{const M=$.target.closest?.(f);if(!M)return;const Pt=$.relatedTarget;Pt&&M.contains(Pt)||w()},Et=$=>{const M=$.target.closest?.(f);M&&b(M)},_t=$=>{$.target.closest?.(f)&&w()};return c.addEventListener("pointerover",x),c.addEventListener("pointerout",St),c.addEventListener("focusin",Et),c.addEventListener("focusout",_t),()=>{clearTimeout(m),c.removeEventListener("pointerover",x),c.removeEventListener("pointerout",St),c.removeEventListener("focusin",Et),c.removeEventListener("focusout",_t),l.hidePopover(),l.remove(),u.current=void 0}},[n,a,s,p]),k(()=>{!n||!u.current||Ot(u.current,r,o)},[r,o,n]),k(()=>{!i||!u.current||u.current.hidePopover()},[i])},Le=t=>{const[e,n]=Zt(!1);return k(()=>{const r=t.current;if(!r)return;const o=()=>{n(r.assignedElements().length>0)};return o(),r.addEventListener("slotchange",o),()=>r.removeEventListener("slotchange",o)},[t.current]),e},Ie=R`
	:host {
...
`,Ue=t=>L(()=>{if(t==null)return;const e=new RegExp(t,"u");return n=>{!n.defaultPrevented&&n.data&&!e.test(n.data)&&n.preventDefault()}},[t]),Qe=T(class extends C{values;constructor(t,e,n,r){super(t,e),Object.assign(e.host,n),this.values=r}update(t,e){this.hasChanged(e)&&(this.values=e,Object.assign(this.state.host,t))}hasChanged(t=[]){return t.some((e,n)=>this.values[n]!==e)}});T(class extends C{update(){return this.state.host}});const Xe=/([A-Z])/gu,At=(t,e,n)=>{t[e]=n,t.dispatchEvent(new CustomEvent(e.replace(Xe,"-$1").toLowerCase()+"-changed",{detail:{value:n}}))},oe=t=>{const e=U(void 0),n=P(u=>e.current=u,[]),r=t.shadowRoot,o=P(u=>t.dispatchEvent(new Event(u.type,{bubbles:u.bubbles})),[]),a=P(u=>At(t,"value",u.target.value),[]),s=P(u=>At(t,"focused",u.type==="focus"),[]),i=P(()=>{const u=e.current?.checkValidity();return t.toggleAttribute("invalid",!u),u},[]);return Qe({validate:i},[i]),k(()=>{const u=g=>{g.composedPath()[0]?.closest?.("input, textarea")||(g.preventDefault(),e.current?.focus())};return r.addEventListener("mousedown",u),()=>r.removeEventListener("mousedown",u)},[]),{onChange:o,onFocus:s,onInput:a,onRef:n}},Ve=({placeholder:t})=>t||" ",Je=(t,e)=>e??(t==="date"?"9999-12-31":void 0),Ke=["type","variant","hint","compact","required","pattern","allowed-pattern","min","max","step","autosize","label","placeholder",...ne],Ze=t=>{const{type:e="text",pattern:n,allowedPattern:r,autocomplete:o,value:a,readonly:s,disabled:i,min:u,max:g,step:p,maxlength:c,required:d}=t,{onChange:l,onFocus:f,onInput:h,onRef:m}=oe(t),b=Ue(r);return t.toggleAttribute("has-value",!!a),ee(y`
			<input
				${X(m)}
		...
		`,t)};customElements.define("cosmoz-input",I(Ze,{observedAttributes:Ke,styleSheets:[K(re)],shadowRootInit:{mode:"open",delegatesFocus:!0}}));const Wt=t=>{t.style.height="",t.style.height=`${t.scrollHeight}px`},tn=(t,e=0)=>{if(e>0){const n=t.getAttribute("rows")??"",r=t.style.height;t.style.height="",t.setAttribute("rows",e),t.style.maxHeight=t.getBoundingClientRect().height+"px",t.style.height=r,t.setAttribute("rows",n)}},en=t=>{const{value:e,maxRows:n}=t,r=L(()=>()=>t.shadowRoot.querySelector("#input"),[]);k(()=>tn(r(),n),[n,r]),k(()=>Wt(r()),[r,e]),k(()=>{const o=r(),a=new ResizeObserver(()=>requestAnimationFrame(()=>Wt(o)));return a.observe(o),()=>a.unobserve(o)},[r])},nn=["rows","placeholder","label","hint","required",...ne],rn=t=>{const{autocomplete:e,value:n,placeholder:r,readonly:o,disabled:a,rows:s,cols:i,maxlength:u}=t,{onChange:g,onFocus:p,onInput:c,onRef:d}=oe(t);return en(t),ee(y`
			<textarea id="input" part="input"
				...
				.value=${ct(n??"")} maxlength=${_(u)} @input=${c}
				@change=${g} @focus=${p} @blur=${p}>`,t)};customElements.define("cosmoz-textarea",I(rn,{observedAttributes:nn,styleSheets:[K(re)],shadowRootInit:{mode:"open",delegatesFocus:!0}}));const on=t=>{const{label:e,value:n,disabled:r,error:o}=t,a=P(s=>t.dispatchEvent(new CustomEvent("change",{detail:s.target.checked})),[]);return y`<input
			id="toggle"
			class="toggle"
			part="toggle"
			type="checkbox"
		...
		<slot name="suffix"></slot>
		${z(o,s=>y`<div class="failure">${s}</div>`)} `},an=R`
	.toggle {
		appearance: none;
