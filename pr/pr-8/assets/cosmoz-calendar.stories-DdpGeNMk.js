...
`,sr={isoBasic:/^\d{4}-\d{2}-\d{2}$/iu},ir=t=>typeof t=="string"&&sr.isoBasic.test(t)?new Date(`${t}T00:00`):new Date(t),H=t=>{if(t==null)return;if(t instanceof Date&&!isNaN(t.getTime()))return t;if(!(typeof t=="number"||typeof t=="string"))return;const e=ir(t);if(!(e instanceof Date&&isNaN(e.getTime())))return e},cr=t=>{const{locale:e,numberOfMonths:n,min:r,max:a}=t,o=e??navigator.language,s=Number(n??1),[c,u]=Pt("start"),[g,l]=Pt("end"),i=_(()=>H(c),[c]),h=_(()=>H(g),[g]),d=_(()=>H(r),[r]),m=_(()=>H(a),[a]),p=_(()=>X(i??new Date,d,m),[i,d,m]),[f,v]=ft(p),[S,z]=ft(!1),[N,A]=ft(p),ce=_(()=>Rn(o),[o]);bt(()=>{S&&t.shadowRoot?.querySelector(`[data-date="${et(N,"yyyy-MM-dd")}"]`)?.focus()},[N,t,S]);const ue=_(()=>{const y=[];for(let k=0;k<s;k++)y.push(Hn(O(f,k),o));return y},[f,o,s]),ut=ht(y=>{const k=et(y,"yyyy-MM-dd");if(!i&&!h){u(k);return}if(i&&!h&&ct(y,i)){u(k),l(et(i,"yyyy-MM-dd"));return}if(i&&!h){l(k);return}i&&h&&(u(k),l(void 0))},[i,h,u,l]),xt=ht(y=>{const k=X(y,d,m);At(k,f)&&v(I=>oe(I,s)),It(k,f,s)&&v(I=>O(I,s)),A(k)},[d,m,s,f,A,v]);bt(()=>{if(At(N,f)){const y=X(wt(f),d,m);A(y)}if(It(N,f,s)){const y=X(ne(O(f,s-1)),d,m);A(y)}},[N,m,d,s,f,A]);const de=ht(y=>{if(!(y.target instanceof Element))return;const k=y.target.closest("[data-date]");if(!k?.dataset.date||k.dataset.disabled==="true")return;const I=H(k.dataset.date);if(y.key==="Enter"||y.key===" "){y.preventDefault(),ut(I);return}const kt=Gn(y,I);kt&&(y.preventDefault(),xt(kt))},[xt,ut]);return{endDate:h,focusedDate:N,locale:o,maxDate:m,minDate:d,monthMatrices:ue,numberOfMonths:s,onClick:ut,onKeyDown:de,selectedMonth:f,setFocusedDate:A,setIsFocused:z,setSelectedMonth:v,startDate:i,weekdayNames:ce}},ur=t=>{const{endDate:e,focusedDate:n,locale:r,maxDate:a,minDate:o,monthMatrices:s,numberOfMonths:c,onClick:u,onKeyDown:g,selectedMonth:l,setFocusedDate:i,setIsFocused:h,setSelectedMonth:d,startDate:m,weekdayNames:p}=cr(t);return x`<div
	...
	</div> `};customElements.define("cosmoz-calendar",Jt(ur,{observedAttributes:["locale","number-of-months"],styleSheets:[Kt,or]}));const E=t=>{const e=new Date;return e.setDate(t),e.toISOString().slice(0,10)},hr={title:"CosmozCalendar",component:"cosmoz-calendar",tags:["autodocs"]},G={name:"sv-SE Two Calendars",render:()=>x`<cosmoz-calendar
            .start=${E(12)}
            .end=${E(17)}
            locale="sv-SE"
            number-of-months="2"
